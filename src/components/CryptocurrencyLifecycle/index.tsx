...
export default function CryptocurrencyLifecycle({
    titleLeft,
    titleRight = null,
    showDetails = true,
}: ComponentProps) {
    ...
    const showDetailsClasslist = showDetails
    ...
    const rowItem = (
        ...
        value: string,
        marker: boolean = true,
        titleYear: boolean = false
    ) => (
        <View style={styles.sectionNested}>
            {marker ? (
                <View style={[markedItemBorderColor, styles.marker]} />
            ) : null}
    ...
    return (
        <View style={wrapper}>
            <View style={styles.section}>
                ...
                {titleRight ? (
                    <Text style={titleLight}>{titleRight}</Text>
                ) : null}
            </View>
